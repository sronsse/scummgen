...
LOFF::LOFF(Game *game, vector<LFLF *> lflfs)
{
	...
	firstOffset += 4 * sizeof(uint8_t); // LOFF identifier
	firstOffset += sizeof(uint32_t); // LOFF size
	firstOffset += sizeof(uint8_t); // LOFF nRooms
	firstOffset += _nRooms * sizeof(uint8_t); // LOFF roomIDs
	firstOffset += _nRooms * sizeof(uint32_t); // LOFF roomOffsets
...
uint32_t LOFF::getSize()
{
	...
	size += _roomOffsets.size() * sizeof(uint32_t); // roomOffsets
	return size;
}
